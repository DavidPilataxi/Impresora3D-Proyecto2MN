...
class Impresora3DSimulada:
    ...
    def centrar_ventana(self):
        ...
        height = self.root.winfo_height()
        x = (self.root.winfo_screenwidth() // 2) - (width // 2)
        y = (self.root.winfo_screenheight() // 5) - (
            height // 5)  # Elevar un poco más la ventana
        self.root.geometry(f'+{x}+{y}')
    ...
    def generar_trayectoria(self):
        ...
        # Crear el polígono para verificar si los puntos están dentro
        poligono_path = Path(self.puntos)

        self.trayectoria = []
        for x in np.arange(min_x, max_x + resolucion, resolucion):
            intersecciones = []
            ...
            intersecciones = sorted(set(intersecciones))
            for i in range(0, len(intersecciones), 2):
                if i + 1 < len(intersecciones):
                    y_vals = np.arange(intersecciones[i],
                                       ...
                                       resolucion)
                    y_vals = np.append(y_vals, intersecciones[i + 1])
                    for y in y_vals:
                        if punto_en_poligono(
                                poligono_path,
                            (x, y)) or (x, y) in self.puntos or y == min_y:
                            self.trayectoria.append((x, y))
